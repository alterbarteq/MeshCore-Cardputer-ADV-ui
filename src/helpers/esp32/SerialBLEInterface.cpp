...
void SerialBLEInterface::onWrite(BLECharacteristic* pCharacteristic) {
  ...
  BLE_DEBUG_PRINTLN("onWrite() called: len=%d", len);
  ...
  if (len > MAX_FRAME_SIZE) {
    ...
    recv_queue_len++;
    BLE_DEBUG_PRINTLN("onWrite() success: queued frame %d", recv_queue_len);
  }
}

void SerialBLEInterface::onWrite(BLECharacteristic* pCharacteristic, esp_ble_gatts_cb_param_t* param) {
  onWrite(pCharacteristic);  // delegate to simpler version
}
