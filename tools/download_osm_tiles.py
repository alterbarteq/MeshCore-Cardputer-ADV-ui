#!/usr/bin/env python3
"""
Originally from d4rkmen/plai (https://github.com/d4rkmen/plai), licensed
GPL-3.0 by that project. Copied here as a standalone, offline PC-side tool —
it is not compiled into or linked with this repo's (MIT-licensed) firmware,
it just produces tile files that get copied onto an SD card. Keep this
notice and the upstream license terms in mind if you redistribute this file.
...
Usage for this fork (PNG tiles, no style subfolder — matches
examples/companion_radio/ui-retro/ScreenMap.h's /maptiles/{z}/{x}/{y}.png):
  python download_osm_tiles.py --lat 52.52 --lon 13.405 --radius 20 \
      --min-zoom 10 --max-zoom 17 --style osm --png --output ./maptiles

  Then copy the resulting "maptiles" folder to the SD card root.
